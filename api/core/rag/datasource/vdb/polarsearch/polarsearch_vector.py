...
logging.getLogger("polarsearch").setLevel(logging.WARN)

ROUTING_FIELD = "routing_field"
UGC_INDEX_PREFIX = "ugc_index"


class PolarSearchVectorStoreConfig(BaseModel):
    hosts: str
...
class PolarSearchVectorStore(BaseVector):
    def __init__(self, collection_name: str, config: PolarSearchVectorStoreConfig, using_ugc: bool, **kwargs):
        self._routing = None
...
class PolarSearchVectorStoreFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> PolarSearchVectorStore:
        polarsearch_config = PolarSearchVectorStoreConfig(
            hosts=dify_config.POLARSEARCH_URL or "",
            username=dify_config.POLARSEARCH_USERNAME,
            password=dify_config.POLARSEARCH_PASSWORD,
        ...
        return PolarSearchVectorStore(index_name, polarsearch_config, routing_value=routing_value, using_ugc=using_ugc)
